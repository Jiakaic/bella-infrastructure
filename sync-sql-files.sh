...
MYSQL_DB="${MYSQL_DB:-bella_file_api}"
...
copy_and_fix_sql_files() {
    ...
    log_success "共处理了 $file_count 个SQL文件"
}
...
execute_sql_files_docker() {
    log_info "通过Docker开始执行SQL文件..."
    
    local success_count=0
    local total_count=0
    
    # 按文件名排序执行
    for sql_file in $(ls "$DEST_DIR"/*.sql 2>/dev/null | grep -v "workflow-" | sort); do
        if [ -f "$sql_file" ]; then
            ((total_count++))
    ...
    log_success "SQL文件执行完成 ($success_count/$total_count)"
}
...
show_help() {
    ...
    echo "  --mysql-db DB       MySQL数据库名 (默认: bella_file_api)"
