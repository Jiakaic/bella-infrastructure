...
if ! command -v mysql &> /dev/null; then
    # 检查是否有Infrastructure MySQL容器运行
    if command -v docker &> /dev/null && docker ps --format "table {{.Names}}" | grep -q "$MYSQL_CONTAINER"; then
        echo -e "${YELLOW}本地未找到MySQL客户端，使用Infrastructure Docker容器执行${NC}"
        ...
        echo -e "${GREEN}docker-compose --env-file .env.infrastructure -f docker-compose.infrastructure.yml up -d mysql${NC}"
        echo ""
...
if ! $MYSQL_CMD -h"${MYSQL_HOST}" -P"${MYSQL_PORT}" -u"${MYSQL_USER}" -p"${MYSQL_PASSWORD}" -e "SHOW TABLES LIKE 'tenant';" "${MYSQL_DATABASE}" 2>/dev/null | grep -q "tenant"; then
...
if [ -n "$current_key" ] && [ "$current_key" != "NULL" ]; then
    ...
    # 检查是否存在test租户
    tenant_count=$($MYSQL_CMD -h"${MYSQL_HOST}" -P"${MYSQL_PORT}" -u"${MYSQL_USER}" -p"${MYSQL_PASSWORD}" -s -N -e "SELECT COUNT(*) FROM tenant WHERE tenant_id = 'test';" "${MYSQL_DATABASE}" 2>/dev/null || echo "0")
    ...
    if [ "$tenant_count" = "0" ]; then
        ...
        if $MYSQL_CMD -h"${MYSQL_HOST}" -P"${MYSQL_PORT}" -u"${MYSQL_USER}" -p"${MYSQL_PASSWORD}" -e "${create_tenant_sql}" "${MYSQL_DATABASE}"; then
